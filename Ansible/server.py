...
# search "try except that gives full error" (+ with traceback)

import sys , re , ssl , socket , threading
import parser as p                      # my parser
import magic
...
def log(req_ip, valid_req):
    time = log_time()
    # Changing the format of the log entry
    log_entry = f"Time: {time} | IP: {req_ip.strip()} | Valid: {valid_req}\n"
    # Using manual file handling
...
def get(resourceIdentifier, httpv):
    ...
    try:
        # Determine body_content and content_type based on the resourceIdentifier
...
def handle_php_request(resourceIdentifier):
    # Extracting PHP file path and parameters
...
def HTTP_response(req, parser_resp):
    ...
    try:
        # Split request into lines and extract the first line
        reqlist = req.split('\r\n')
...
try:
    ...
    if arg_count == 3:
        ...
        print("HTTPS server:    python3 server.py <IP> <PORT> <CRT File> <Key File>\n")
